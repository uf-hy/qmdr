...
function resolveSqliteVecFallbackPaths(): string[] {
  ...
  // Bun bundle/runtime-relative search: walk upward to find node_modules/<pkg>/vec0.dylib
  try {
...
function initializeDatabase(db: Database): void {
  ...
  // Triggers to keep FTS in sync (DROP first to apply schema changes on existing DBs)
...
function chunkDocumentByChars(
  ...
  const CHARS_PER_TOKEN = 3.5; // Approximate for English/mixed text
  const maxChars = Math.floor(maxTokens * CHARS_PER_TOKEN);
  const overlapChars = Math.floor(overlapTokens * CHARS_PER_TOKEN);

  if (content.length <= maxChars) {
    return [{ text: content, pos: 0, tokens: Math.ceil(content.length / CHARS_PER_TOKEN) }];
  }
  ...
  while (charPos < content.length) {
    ...
    if (chunkEnd < content.length) {
      ...
      const breakMatch = searchSlice.match(/\n\n|\n(?=[A-Z#])|[.!?]\s+/);
      if (breakMatch && breakMatch.index !== undefined) {
        const breakPos = searchStart + breakMatch.index + breakMatch[0].length;
    ...
    charPos += step;
    if (charPos >= content.length) break;
  }
...
function splitCJKTerms(text: string): string[] {
  // Split CJK text into trigrams (3-char) for trigram FTS matching
  // FTS5 trigram tokenizer requires ≥3 characters per term
  const cjkRanges = /[\u4e00-\u9fff\u3400-\u4dbf\uf900-\ufaff]/;
  const results: string[] = [];
  
  for (const word of text.split(/\s+/)) {
    const sanitized = sanitizeFTS5Term(word);
    if (!sanitized) continue;
    
    if (cjkRanges.test(sanitized)) {
      // CJK: extract trigrams (3 consecutive characters) to match FTS5 trigram tokenizer
      const chars = [...sanitized].filter(c => cjkRanges.test(c));
      if (chars.length <= 3) {
        // Short CJK (1-3 chars): use as-is (3 chars = exact trigram match)
        results.push(chars.join(''));
      } else {
        // Longer CJK: extract overlapping trigrams
        for (let i = 0; i <= chars.length - 3; i++) {
          results.push(chars[i]! + chars[i + 1]! + chars[i + 2]!);
        }
      }
    } else {
...
function buildFTS5Query(query: string): string | null {
  const terms = splitCJKTerms(query).filter(t => t.length > 0);
  if (terms.length === 0) return null;
  // Use OR for CJK bigrams (any bigram match is useful), AND for mixed
  // Deduplicate terms
  const unique = [...new Set(terms)];
  if (unique.length === 1) return `"${unique[0]}"*`;
  // For CJK-heavy queries, use OR to maximize recall (reranker handles precision)
  const hasCJK = unique.some(t => /[\u4e00-\u9fff]/.test(t));
  if (hasCJK) {
...
export async function searchVec(db: Database, query: string, model: string, limit: number = 20, collectionName?: string, session?: ILLMSession): Promise<SearchResult[]> {
  ...
  // Combine with distances and keep chunk-level results (no file-level dedupe)
  return docRows
    .map((row) => ({ row, bestDist: distanceMap.get(row.hash_seq) ?? 1 }))
...
// Cached remote embedder for store.ts (initialized lazily from env vars)
let _remoteEmbedder: RemoteLLM | null | undefined = undefined;
function getRemoteEmbedder(): RemoteLLM | null {
  if (_remoteEmbedder !== undefined) return _remoteEmbedder;
  ...
  _remoteEmbedder = new RemoteLLM({
    ...
    siliconflow: {
      ...
      embedModel: process.env.QMD_SILICONFLOW_EMBED_MODEL || 'Qwen/Qwen3-Embedding-8B',
    ...
    openai: (oaApiKey || (rerankProvider === 'openai' && sfApiKey)) ? {
